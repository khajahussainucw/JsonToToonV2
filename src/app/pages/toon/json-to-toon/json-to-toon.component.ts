...
@Component({
  selector: 'app-json-to-toon',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './json-to-toon.component.html',
  styleUrl: './json-to-toon.component.css'
})
export class JsonToToonComponent implements AfterViewInit {
  ...
  private setupMetaTags() {
    this.title.setTitle('JSON to TOON Converter - Convert JSON to TOON Online');
    this.meta.updateTag({
      name: 'description',
      content: 'Convert JSON to TOON (Token-Oriented Object Notation) online. Transform JSON to TOON format for LLM token efficiency - up to 60% fewer tokens than JSON.'
    });
    this.meta.updateTag({
      name: 'keywords',
      content: 'json to toon, toon converter, convert json to toon, json to toon online, token oriented object notation, llm format'
    });
  }
  ...
  private initializeEditors() {
    ...
    // Initialize output editor for TOON
  ...
  private debouncedConvertJson() {
    clearTimeout(this.debounceTimer);
    this.debounceTimer = setTimeout(() => {
      this.convertJsonToToon();
    }, 300);
  }

  private convertJsonToToon() {
    try {
      ...
      const toonOutput = encode(parsedJson);

      this.aceOutputEditor.setValue(toonOutput, -1);
  ...
  copyToClipboard(): void {
    ...
    if (!outputContent || outputContent.trim() === '' || outputContent.startsWith('Invalid JSON') || outputContent.startsWith('Error:')) {
    ...
    navigator.clipboard.writeText(outputContent).then(() => {
      this.copySuccessMessage = 'TOON copied to clipboard!';
  ...
  downloadToon(): void {
    ...
    link.href = url;
    link.download = 'converted.toon';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);
    
    this.copySuccessMessage = 'TOON downloaded successfully!';
    this.copySuccessVisible = true;
    setTimeout(() => {
      this.copySuccessVisible = false;
    }, 3000);
