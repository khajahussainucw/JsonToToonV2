...
@Component({
  selector: 'app-toon-validator',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './toon-validator.component.html',
  styleUrl: './toon-validator.component.css'
})
export class ToonValidatorComponent implements AfterViewInit {
  ...
  private setupMetaTags() {
    this.title.setTitle('TOON Validator - Validate TOON Syntax Online');
    this.meta.updateTag({
      name: 'description',
      content: 'Validate TOON (Token-Oriented Object Notation) syntax online with our fast TOON validator tool. Check for syntax errors and formatting issues in your TOON files.'
    });
    this.meta.updateTag({
      name: 'keywords',
      content: 'toon validator, toon validation, validate toon, toon syntax checker, toon lint, toon online validator, token oriented object notation validator'
    });
  }
  ...
  private validateToon() {
    const inputToon = this.aceInputEditor.getValue().trim();
    
    if (!inputToon) {
      this.validationResult = { valid: false, message: 'Please enter TOON to validate' };
    ...
    try {
      // Decode TOON to validate it
      const decoded = decode(inputToon);
      
      // Build success message
      let outputMessage = '✓ VALID TOON\n\n';
      ...
      if (decoded === null || decoded === undefined) {
        ...
        if (typeof decoded === 'object') {
          if (Array.isArray(decoded)) {
            ...
            outputMessage += `Item Count: ${decoded.length}\n`;
          } else {
          ...
          outputMessage += `Document Type: ${typeof decoded}\n`;
          outputMessage += `Value: ${decoded}`;
        }
      }
      ...
      const positionMatch = error.message.match(/position (\d+)/i) || error.message.match(/line (\d+)/i);
      if (positionMatch) {
        const position = parseInt(positionMatch[1], 10);
        const textBeforeError = inputToon.substring(0, position);
        lineNumber = (textBeforeError.match(/\n/g) || []).length + 1;
      }
      
      let errorMessage = '✗ INVALID TOON\n\n';
      errorMessage += `Error: ${error.message || 'Invalid TOON syntax'}\n`;
      if (lineNumber) {
        errorMessage += `Approximate Line: ${lineNumber}\n`;
      }
      errorMessage += `\nCommon TOON errors:\n`;
      errorMessage += `  - Missing array length in header\n`;
      errorMessage += `  - Mismatched row count\n`;
      errorMessage += `  - Invalid delimiter usage\n`;
      errorMessage += `  - Incorrect indentation\n`;
      
      this.validationResult = { 
        valid: false, 
        message: error.message || 'Invalid TOON',
        error: error,
  ...
  copyToClipboard(): void {
    ...
    navigator.clipboard.writeText(outputContent).then(() => {
      this.copySuccessMessage = 'Validation result copied to clipboard!';
