...
@Component({
  selector: 'app-yaml-to-typescript',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './yaml-to-typescript.component.html',
  styleUrl: './yaml-to-typescript.component.css'
})
export class YamlToTypescriptComponent implements AfterViewInit {
  ...
  private setupMetaTags() {
    this.title.setTitle('YAML to TypeScript Converter - Generate TypeScript Interfaces');
    this.meta.updateTag({
      name: 'description',
      content: 'Convert YAML to TypeScript interfaces online. Generate TypeScript types from YAML data instantly for type-safe development.'
    });
    this.meta.updateTag({
      name: 'keywords',
      content: 'yaml to typescript, yaml to ts, typescript interface generator, yaml to interface, typescript types from yaml'
    });
  }
  ...
  private debouncedConvert() {
    ...
    this.debounceTimer = setTimeout(() => {
      this.convertYamlToTypeScript();
    }, 300);
  }

  private convertYamlToTypeScript() {
    try {
      ...
      const typescript = this.generateTypeScript(parsedYaml, this.interfaceName);
