...
@Component({
  selector: 'app-json-to-php',
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule],
  templateUrl: './json-to-php.component.html',
  styleUrl: './json-to-php.component.css'
})
export class JsonToPhpComponent implements AfterViewInit {
  ...
  private setupMetaTags() {
    this.title.setTitle('JSON to PHP Converter - Generate PHP Classes');
    this.meta.updateTag({
      name: 'description',
      content: 'Convert JSON to PHP classes online. Generate PHP classes from JSON data instantly for PHP development.'
    });
    this.meta.updateTag({
      name: 'keywords',
      content: 'json to php, json to php class, php class generator, json to php object, php code generator'
    });
  }
  ...
  private initializeEditors() {
    ...
    // Initialize output editor for PHP
  ...
  private convertJsonToPhp() {
    try {
      ...
      const php = this.generatePhp(parsedJson, this.className);
  ...
  private generatePhp(obj: any, className: string): string {
    ...
    const generateClass = (value: any, className: string, indent: string = ''): string => {
      ...
      if (Array.isArray(value)) {
        ...
        const itemType = this.getPhpType(firstItem);
        return `${itemType}[]`;
      }
      
      if (typeof value !== 'object' || value === null) {
        return this.getPhpType(value);
      }
      ...
      for (const key in value) {
        ...
        if (propValue === null) {
          propType = this.useNullableTypes ? '?mixed' : 'mixed';
          ...
          propType = this.getPhpType(propValue);
        }
        ...
        if (this.useTypedProperties) {
          properties.push(`${indent}    ${nullable}${propType} $${propName};`);
        } else {
          properties.push(`${indent}    public $${propName};`);
        }
      }
      
      let classStr = '<?php\n\n';
      if (this.namespace) {
        classStr += `namespace ${this.namespace};\n\n`;
      }
  ...
  private getPhpType(value: any): string {
    if (value === null) {
      return this.useNullableTypes ? '?mixed' : 'mixed';
    }
  ...
  copyToClipboard(): void {
    ...
    navigator.clipboard.writeText(outputContent).then(() => {
      this.copySuccessMessage = 'PHP code copied to clipboard!';
  ...
  downloadPhp(): void {
    ...
    this.copySuccessMessage = 'PHP file downloaded successfully!';
    this.copySuccessVisible = true;
    setTimeout(() => {
      this.copySuccessVisible = false;
    }, 3000);
