...
export class JsonToDartComponent implements AfterViewInit {
  ...
  private setupMetaTags() {
    this.title.setTitle('JSON to Dart Converter - Generate Dart Classes');
    this.meta.updateTag({
      name: 'description',
      content: 'Convert JSON to Dart classes online. Generate Dart classes with json_serializable from JSON data instantly for Flutter development.'
    });
    this.meta.updateTag({
      name: 'keywords',
      content: 'json to dart, json to dart class, dart class generator, json to flutter, dart code generator'
    });
  }
  ...
  private initializeEditors() {
    ...
    this.aceOutputEditor.session.setMode('ace/mode/dart');
  ...
  private convertJsonToDart() {
    try {
      ...
      const dart = this.generateDart(parsedJson, this.className);
      
      this.aceOutputEditor.setValue(dart, -1);
  ...
  private generateDart(obj: any, className: string): string {
    ...
    const generateClass = (value: any, className: string, indent: string = ''): string => {
      ...
      if (Array.isArray(value)) {
        ...
        const itemType = this.getDartType(firstItem);
        return `List<${itemType}>`;
      }
      
      if (typeof value !== 'object' || value === null) {
        return this.getDartType(value);
      }
      ...
      for (const key in value) {
        ...
        if (propValue === null) {
          propType = this.useNullableTypes ? 'dynamic?' : 'dynamic';
        } else if (Array.isArray(propValue)) {
          if (propValue.length === 0) {
            propType = 'List<dynamic>';
          } else {
          ...
          propType = this.getDartType(propValue);
        }
        
        const nullable = (propValue === null && this.useNullableTypes) ? '?' : '';
        const propertyName = this.camelCase(key);
        properties.push(`${indent}  final ${propType}${nullable} ${propertyName};`);
      }
      
      let classStr = '';
      if (this.useJsonSerializable) {
        classStr += `import 'package:json_annotation/json_annotation.dart';\n\n`;
        classStr += `part '${className.toLowerCase()}.g.dart';\n\n`;
        classStr += `@JsonSerializable()\n`;
      }
      classStr += `${indent}class ${className} {\n`;
      classStr += properties.join('\n');
      classStr += `\n\n${indent}  ${className}({${properties.map(p => p.split(' ').pop()?.replace(';', '')).join(', ')}});\n`;
      if (this.useJsonSerializable) {
        classStr += `\n${indent}  factory ${className}.fromJson(Map<String, dynamic> json) => _$${className}FromJson(json);\n`;
        classStr += `${indent}  Map<String, dynamic> toJson() => _$${className}ToJson(this);\n`;
      }
      classStr += `${indent}}`;
  ...
  private getDartType(value: any): string {
    if (value === null) {
      return this.useNullableTypes ? 'dynamic?' : 'dynamic';
    }
  ...
  copyToClipboard(): void {
    ...
    navigator.clipboard.writeText(outputContent).then(() => {
      this.copySuccessMessage = 'Dart code copied to clipboard!';
  ...
  downloadDart(): void {
    ...
    link.href = url;
    link.download = `${this.className.toLowerCase()}.dart`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);
    
    this.copySuccessMessage = 'Dart file downloaded successfully!';
    this.copySuccessVisible = true;
    setTimeout(() => {
      this.copySuccessVisible = false;
    }, 3000);
