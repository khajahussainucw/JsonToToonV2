...
export class JsonToKotlinComponent implements AfterViewInit {
  ...
  private setupMetaTags() {
    this.title.setTitle('JSON to Kotlin Converter - Generate Kotlin Data Classes');
    this.meta.updateTag({
      name: 'description',
      content: 'Convert JSON to Kotlin data classes online. Generate Kotlin data classes from JSON data instantly for Android development.'
    });
    this.meta.updateTag({
      name: 'keywords',
      content: 'json to kotlin, json to kotlin data class, kotlin data class generator, json to kotlin class, kotlin code generator'
    });
  }
  ...
  private initializeEditors() {
    ...
    this.aceOutputEditor.session.setMode('ace/mode/kotlin');
  ...
  private convertJsonToKotlin() {
    try {
      ...
      const kotlin = this.generateKotlin(parsedJson, this.dataClassName);
  ...
  private generateKotlin(obj: any, className: string): string {
    ...
    const generateClass = (value: any, className: string, indent: string = ''): string => {
      ...
      if (Array.isArray(value)) {
        ...
        const itemType = this.getKotlinType(firstItem);
        return `List<${itemType}>`;
      }
      
      if (typeof value !== 'object' || value === null) {
        return this.getKotlinType(value);
      }
      ...
      for (const key in value) {
        ...
        if (propValue === null) {
          propType = this.useNullableTypes ? 'Any?' : 'Any';
          ...
          propType = this.getKotlinType(propValue);
        }
        
        const nullable = (propValue === null && this.useNullableTypes) ? '?' : '';
        const propertyName = this.camelCase(key);
        properties.push(`${indent}    val ${propertyName}: ${propType}${nullable}`);
      }
      
      let classStr = '';
      if (this.useDataClass) {
        classStr += `${indent}data class ${className}(\n`;
  ...
  private getKotlinType(value: any): string {
    if (value === null) {
      return this.useNullableTypes ? 'Any?' : 'Any';
    }
  ...
  copyToClipboard(): void {
    ...
    navigator.clipboard.writeText(outputContent).then(() => {
      this.copySuccessMessage = 'Kotlin code copied to clipboard!';
  ...
  downloadKotlin(): void {
    ...
    link.href = url;
    link.download = `${this.dataClassName}.kt`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    window.URL.revokeObjectURL(url);
    
    this.copySuccessMessage = 'Kotlin file downloaded successfully!';
    this.copySuccessVisible = true;
    setTimeout(() => {
      this.copySuccessVisible = false;
    }, 3000);
